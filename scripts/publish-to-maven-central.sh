...
# Check if Maven settings are configured
if [ ! -f ~/.m2/settings.xml ]; then
    echo "Error: Maven settings file not found at ~/.m2/settings.xml"
    echo "Please configure your Sonatype credentials in Maven settings"
    exit 1
fi

# Check if OSSRH server is configured in settings.xml
if ! grep -q "<id>ossrh</id>" ~/.m2/settings.xml; then
    echo "Error: OSSRH server not configured in ~/.m2/settings.xml"
    echo "Please add your Sonatype credentials to Maven settings"
    exit 1
fi

echo "✅ Maven settings configured for Sonatype publishing"
...
# Skip tests for publishing
echo "2. Skipping tests for publishing..."
